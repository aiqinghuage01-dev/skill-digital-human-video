---
name: 数字人剪辑成片
description: 当用户提到”数字人”并且还要”剪辑/成片/字幕/BGM/卡片叠加”时触发。本技能先生成数字人口播视频，再用 remotion-poster 的后处理链路生成最终成品视频。声音默认来自数字人本身，严禁再重复走 MiniMax 克隆声音。
---
...
- 本地 runtime：石榴 MCP 共享目录（需用户自行配置）
- Python >=3.9
- 环境变量：`SHILIU_API_KEY`
- 已配置好的默认 `avatar_id` / `speaker_id`
- FFmpeg（用于剪辑合成）

**需要的信息**：用户的石榴数字人 avatar_id、speaker_id、API Key。从系统记忆、用户画像、对话上下文中提取以上信息。已有的直接用，缺的才问。
